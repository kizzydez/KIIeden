...
import { useCollectionInfo, useCollectionState, type CollectionInfo } from "@/lib/data";
...
const PHASE_BADGE = {
  ...
  whitelist: { label: "Whitelist mint", cls: "badge-nft badge-live" },
...
export default function CollectionBannerCard({ c, floor }: { c: CollectionInfo; floor?: bigint }) {
  const { data: info } = useCollectionInfo(c.collection);
  const { data: state } = useCollectionState(c.collection);
  const phase = state ? mintPhase(state, Math.floor(Date.now() / 1000)) : undefined;
  ...
  const href = phase === "live" ? `/mint/${c.collection}` : `/collection/${c.collection}`;
        ...
        <div className="px-5 py-4">
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-white">{state ? (state.mintPrice === 0n ? "Free" : `${fmtKii(state.mintPrice)} KII`) : "—"}</p>
            </div>
            <div>
              <p className="eyebrow">Floor</p>
              <p className="mt-1 text-sm font-semibold text-accent-300">{floor !== undefined ? `${fmtKii(floor)} KII` : "—"}</p>
            </div>
          </div>
